...
class StudentAttendanceLogResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        if ($this->event_type?->value === 'check_in' && $this->scanned_at) {
            if ($schedule) {
                $scannedAt = CarbonImmutable::instance($this->scanned_at)->timezone($timezone);
                $lateStartsAt = CarbonImmutable::parse($scannedAt->format('Y-m-d').' '.$schedule->time_in, $timezone)
                    ->addMinutes(20);

                if ($scannedAt->gt($lateStartsAt)) {
                    $scheduleStatus = 'Late';
                }
            }
        ...
        return [
            ...
            'scanned_at_display' => $this->scanned_at?->timezone($timezone)->format('h:i A'),
            'scanned_at_full_display' => $this->scanned_at?->timezone($timezone)->format('M d, Y h:i A'),

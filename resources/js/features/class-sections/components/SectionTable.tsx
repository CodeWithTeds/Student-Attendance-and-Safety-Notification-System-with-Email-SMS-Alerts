...
export default function SectionTable({
    ...
    const handleDelete = (section: Section) => {
        if (
            confirm(
                `Delete ${section.grade_level?.name ?? 'this grade'} - ${section.name}?`,
            )
        ) {
            ...
            <table className="w-full min-w-[1080px] border-collapse text-[13px]">
                <thead>
                    <tr className="border-b-2 border-[var(--border)]">
                        <th className="sticky top-0 z-10 bg-[var(--background)] px-2 py-3 text-left text-[11px] font-semibold tracking-wider text-[var(--muted-foreground)] uppercase">
                            #
                        </th>
                        <th className="sticky top-0 z-10 bg-[var(--background)] px-2 py-3 text-left text-[11px] font-semibold tracking-wider text-[var(--muted-foreground)] uppercase">
                            Section
                        ...
                        <th className="sticky top-0 z-10 bg-[var(--background)] px-2 py-3 text-right text-[11px] font-semibold tracking-wider text-[var(--muted-foreground)] uppercase">
                            Actions
                        </th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-[var(--border)]">
                    {sections.length === 0 ? (
                        ...
                        sections.map((section, index) => {
                            const capacityLabel = section.capacity
                                ...
                                >
                                    ...
                                    <td className="px-2 py-2.5">
                                        <div className="flex items-center justify-end gap-1">
                                            <button
                                                className="flex h-7 w-7 items-center justify-center rounded-lg text-[var(--muted-foreground)] transition-colors hover:bg-sky-50 hover:text-sky-600"
                                                onClick={() =>
                                                    onSchedule(section)
                                                }
                                                title="Edit schedule"

...
import { HeartPulse, Mail, MessageSquareText, Plus, X } from 'lucide-react';
import { FormEvent } from 'react';
import { GuardianForm, StudentOption } from '../types';
...
const initialForm: GuardianForm = {
    name: '',
    email: '',
    password: '',
...
export default function AddGuardianModal({ isOpen, onClose, students }: Props) {
    const {
        data: form,
        setData,
        post,
        ...
        reset,
        clearErrors,
    } = useForm<GuardianForm>(initialForm);
            ...
            >
                <div className="flex items-start justify-between gap-4 border-b border-[var(--border)] p-5">
                    <div>
                        <div className="flex items-center gap-2.5 text-lg font-bold text-[var(--foreground)]">
                            <HeartPulse className="text-[var(--primary)]" size={22} />
                            Add Parent / Guardian
                        </div>
                        <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                            Create a guardian account and choose notification channels.
                        </p>
                    </div>
                    <button
                        type="button"
                        ...
                        aria-label="Close add guardian modal"
                    >
                        <X size={16} />
                    </button>
                </div>
                ...
                <div className="overflow-y-auto p-6">
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-semibold text-[var(--foreground)]">SMS contact number</label>
                            <input
                                className="h-10 rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 text-sm text-[var(--foreground)] outline-none transition-all focus:border-[var(--primary)] focus:ring-4 focus:ring-[var(--primary)]/5"
                                value={form.guardian_phone}
                                onChange={(event) => setData('guardian_phone', event.target.value)}
                                placeholder="09XXXXXXXXX"
                            />
                            {errors.guardian_phone && <span className="text-[11px] font-medium text-red-500">{errors.guardian_phone}</span>}
                        </div>
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <input
                                ...
                                required
                            />
                            {errors.password && <span className="text-[11px] font-medium text-red-500">{errors.password}</span>}
                        </div>
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <input
                                ...
                                required
                            />
                            {errors.password_confirmation && <span className="text-[11px] font-medium text-red-500">{errors.password_confirmation}</span>}
                        </div>

                        <div className="flex flex-col gap-2 md:col-span-2">
                            <label className="text-xs font-semibold text-[var(--foreground)]">Notifications</label>
                            <div className="flex flex-wrap gap-3">
                                <label className="inline-flex h-10 items-center gap-2 rounded-lg border border-[var(--border)] px-3 text-sm text-[var(--foreground)]">
                                    <input
                                        ...
                                        onChange={(event) => setData('notification_sms_enabled', event.target.checked)}
                                    />
                                ...
                                <label className="inline-flex h-10 items-center gap-2 rounded-lg border border-[var(--border)] px-3 text-sm text-[var(--foreground)]">
                                    <input
                                        ...
                                        onChange={(event) => setData('notification_email_enabled', event.target.checked)}
                                    />
                                    <Mail size={14} />
                                    Email
                                </label>
                        ...
                        <div className="flex flex-col gap-2 md:col-span-2">
                            <label className="text-xs font-semibold text-[var(--foreground)]">Linked students</label>
                            <div className="max-h-44 overflow-y-auto rounded-lg border border-[var(--border)] p-3">
                                {students.length === 0 ? (
                                    <p className="text-xs text-[var(--muted-foreground)]">No students available.</p>
                                ) : (
                            ...
                            {errors.student_ids && <span className="text-[11px] font-medium text-red-500">{errors.student_ids}</span>}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-2 border-t border-[var(--border)] p-4">
                    ...
                    >
                        <Plus size={14} />
                        {processing ? 'Saving...' : 'Add Guardian'}
                    </button>
                </div>
            </form>
        </div>
